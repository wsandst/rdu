# Test script which runs du and mdu on various different folders
...
echo "[TEST] Running du and mdu comparison tests..."

for path in $@
do
    du_result=`du -s -l -B1 $path | head -n1 | awk '{print $1;}'`
    mdu_result=`build/debug/mdu -j 2 -B1 $path | head -n1 | awk '{print $1;}'`
    if [ "$du_result" -eq "$mdu_result" ]; then
        echo -e "[TEST] '${path}': ${GREEN} OK ${CLEAR}"
    else
        echo -e "[TEST] '${path}': ${RED} FAIL${CLEAR}"
        echo "du: ${du_result}, mdu: ${mdu_result}"
        failed_test=true
    fi
done
